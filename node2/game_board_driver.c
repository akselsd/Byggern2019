...
#define ENCODER_RANGE 8700
#define SCALING_FACTOR 34.1176
#define KP 1
#define KI 1
#define KD 1
...
static motor_input get_motor_input(uint8_t target)
{
	/* Map 0 to 8700 to 0 - 255 */
	float reference = (128 - target)/ 2;
	...
	printf("Current position: %d. Target %d -- ", (int16_t)current_position, (int16_t)reference);
	if (input < 0)
	{
...
{
	//printf("Slider values: Left %u, Right %u\n", msg->data[0], msg->data[1]);
	if (msg->data[0] < 200){
		motor_box_set_speed(0);
		return;
	}
	get_motor_input(msg->data[1]);
